...
public class PostService {
    ...
    public PostResponseDto inquire(Long id) {
        ...
        return new PostResponseDto(post, findPostId(id), voteRepository.countByPostIdAndVote(post, true),
                voteRepository.countByPostIdAndVote(post, false));
    }

    // 게시글 수정
    @Transactional
    public PostResponseDto update(PostUpdateRequestDto postUpdateRequestDto, List<MultipartFile> multipartFiles,
                                  List<Long> deleteId) {
        ...
        if (user.equals(post.getUserId())) {
            ...
            return new PostResponseDto(post, findPostId(post.getId()), voteRepository.countByPostIdAndVote(post, true),
                    voteRepository.countByPostIdAndVote(post, false));
        } else {
    ...
    public List<PostResponseDto> inquireLatest(String todo) {
        ...
        for (Post post : postList) {
            postResponseDtoList.add(
                    new PostResponseDto(post, findPostId(post.getId()), voteRepository.countByPostIdAndVote(post, true),
                            voteRepository.countByPostIdAndVote(post, false)));
        }
